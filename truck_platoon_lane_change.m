...
%Truck platoon performs a coordinated lane change to circumvent an obstacle, 
% utilizing sensors and synchronized movements.
...
truck_1_init_speed  = 100/3.6;   % Speed of the leading truck            [m/s]
% Initial position of the leading truck [m]
truck_1_init_pos    = distance_analysis-20;      

% Truck 2 (Second truck)
truck_2_length      = 20;     % Length of the second truck            [m]
truck_2_width       = 3.0;      % Width of the second truck             [m]
truck_2_init_speed  = 100/3.6;   % Speed of the second truck             [m/s]
...
truck_3_init_speed  = 100/3.6;   % Speed of the third truck              [m/s]
...
truck_4_init_speed  = 100/3.6;   % Speed of the fourth truck              [m/s]
...
parameters.truck_4_width    = truck_4_width;
% Car1
...
% Initial lateral positions (assuming all trucks start in the same lane)
truck_1_init_lat_pos = 0; % You can adjust this depending on the lane
truck_2_init_lat_pos = 0;
truck_3_init_lat_pos = 0;
truck_4_init_lat_pos = 0;
...
states_initial_condition = [truck_1_init_pos 
                            ...
                            car2_init_speed
                            car2_init_lat_pos
                            truck_1_init_lat_pos
                            truck_2_init_lat_pos
                            truck_3_init_lat_pos
                            truck_4_init_lat_pos];
...
% Assuming new state indices are 15 to 18
truck_1_lat_position = YOUT(:, 15);
truck_2_lat_position = YOUT(:, 16);
truck_3_lat_position = YOUT(:, 17);
truck_4_lat_position = YOUT(:, 18);
...
for i=1:length(time)
    ...
    subplot(3,2,5:6)
        ...
        %Trucks Lateral position
        truck_1_lat_pos_inst = truck_1_lat_position(i);
        truck_2_lat_pos_inst = truck_2_lat_position(i);
        truck_3_lat_pos_inst = truck_3_lat_position(i);
        truck_4_lat_pos_inst = truck_4_lat_position(i);

        % ROAD MARKINGS
        sideMarkingsX = [car_pos_lon_inst-distance_analysis car_pos_lon_inst];
        set(gca,'xlim',[car_pos_lon_inst-distance_analysis car_pos_lon_inst],'ylim',[-trackWidth/2-laneMargin +trackWidth/2+laneMargin])
        ...
        truck_1_dimension_Y = [truck_1_lat_pos_inst+truck_1_width/2 truck_1_lat_pos_inst-truck_1_width/2 truck_1_lat_pos_inst-truck_1_width/2 truck_1_lat_pos_inst+truck_1_width/2];
        % Truck 2
        ...
        truck_2_dimension_Y = [truck_2_lat_pos_inst+truck_2_width/2 truck_2_lat_pos_inst-truck_2_width/2 truck_2_lat_pos_inst-truck_2_width/2 truck_2_lat_pos_inst+truck_2_width/2];
        % Truck 3
        ...
        truck_3_dimension_Y = [truck_3_lat_pos_inst+truck_3_width/2 truck_3_lat_pos_inst-truck_3_width/2 truck_3_lat_pos_inst-truck_3_width/2 truck_3_lat_pos_inst+truck_3_width/2];
        % Truck 4
        ...
        truck_4_dimension_Y = [truck_4_lat_pos_inst+truck_4_width/2 truck_4_lat_pos_inst-truck_4_width/2 truck_4_lat_pos_inst-truck_4_width/2 truck_4_lat_pos_inst+truck_4_width/2];
        % Car
        ...
        if car_rear_right_corner_lateral_pos < truck_2_width/2 && car_rear_right_corner_lateral_pos>=-2
...
function dz = simulation(t,z,parameters)
    ...
    car2_length      = parameters.car2_length;
    ...
    %Truck lateral position
    truck_1_pos_lat = z(15);
    truck_2_pos_lat = z(16);
    truck_3_pos_lat = z(17);
    truck_4_pos_lat = z(18);
    
    % SENSORS
    Truck_1_sensors.distance_preceding  = (car_pos_lon-car_length) - truck_2_position;
    Truck_1_sensors.speed_preceding     = car_speed;
    ...
    % DYNAMIC MODELS
    % Truck 1
    truck_1_derivative_states = truck_model(t,truck_1_states,parameters,1,Truck_1_sensors);
   ...
   % Lateral speed car (kinematic model)
    dz(11,1) = 0;
    dz(14,1) = 0; % lateral speed [m/s]

    % Append derivatives for lateral positions of truck
    if truck_1_pos_lat <= -4.3 && car2_pos_lat>=2
        dz(15,1) = 0;
    else
        dz(15,1) = -3.0; % lateral speed [m/s]
    end

    if truck_1_pos_lat <= -2.1 && truck_2_pos_lat >= -4.3 && car2_pos_lat>=2
        dz(16,1) = -3.0;
    else
        dz(16,1) = 0; % lateral speed [m/s]
    end

    if truck_2_pos_lat <= -2.1 && truck_3_pos_lat >= -4.3 && car2_pos_lat>=2
        dz(17,1) = -3.0;
    else
        dz(17,1) = 0; % lateral speed [m/s]
    end

    if truck_3_pos_lat <= -2.1 && truck_4_pos_lat >= -4.3 && car2_pos_lat>=2
        dz(18,1) = -3.0;
    else
        dz(18,1) = 0; % lateral speed [m/s]
    end

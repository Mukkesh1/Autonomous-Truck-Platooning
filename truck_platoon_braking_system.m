%% Scenario 3
% Simulation and animation of a four-truck platooning at 72km/h with 1 second time gap.
% Truck platoon executes coordinated emergency braking in response to a car coming to halt, 
% demonstrating the effectiveness of integrated collision avoidance systems in maintaining convoy safety.
...
function dz = simulation(t,z,parameters)
    ...
    if car_rear_right_corner_lateral_pos==0
        car_speed = 0.0;
    else
        car_speed           = z(10);
    end
    ...
    % DYNAMIC MODELS
    % Truck 1
    truck_1_derivative_states = truck_model(t,truck_1_states,parameters,1,1, car_speed);
    truck_2_derivative_states = truck_model(t,truck_2_states,parameters,2,Truck_2_sensors,car_speed);
    truck_3_derivative_states = truck_model(t,truck_3_states,parameters,3,Truck_3_sensors,car_speed);
    truck_4_derivative_states = truck_model(t,truck_4_states,parameters,4,Truck_4_sensors, car_speed);
    ...
    if car_pos_lat <= 0
        ...
        dz(11,1) = -1.0; % lateral speed [m/s]
    end
        dz(14,1) = 0;

end

function dstates = truck_model(t,states,parameters,truck_flag,truck_sensors, car_speed)
...
function dstates = car_model(t,states)
...
%     X = states(1);
    ...
    if t >=  5 % Time after which the car starts stopping
        V_r = 0;                % Reference speed           [m/s]
    else
        V_r = 20;               % Reference speed           [m/s]
    end                   % Reference speed           [m/s]
    Kp  = 500;                  % Controller gain
    Ft  = Kp*(V_r - V) + Dx;    % Longitudinal force        [N]
